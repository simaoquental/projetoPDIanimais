...
export default function Admin() {
  ...
  const eliminarEntidade = async (tabela: string, idCampo: string, id: string) => {
    if (tabela === 'utilizadores') {
      const user = listaUsers.find(u => u.id_utilizador === id);
        ...
        <div className="fixed inset-0 bg-[#0f172a]/95 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[3rem] w-full max-w-2xl p-12 shadow-2xl relative animate-in zoom-in-95 duration-300 overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleGuardarAssoc} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Morada</label>
                <input value={form.morada} onChange={e => setForm({ ...form, morada: e.target.value })} className="w-full mt-2 bg-slate-50 border-none rounded-2xl p-4 font-bold outline-none" />
              </div>
              ...
              <div>
                ...
                <input value={form.codigo_postal} onChange={e => setForm({ ...form, codigo_postal: e.target.value })} className="w-full mt-2 bg-slate-50 border-none rounded-2xl p-4 font-bold outline-none" />
              </div>

              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Telefone</label>
                <input type="tel" maxLength={9} value={form.telefone} onChange={e => setForm({ ...form, telefone: e.target.value.replace(/\D/g, '').slice(0, 9) })} className="w-full mt-2 bg-slate-50 border-none rounded-2xl p-4 font-bold outline-none" />
              </div>
