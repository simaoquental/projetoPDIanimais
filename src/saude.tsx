...
export default function HistoricoSaude() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.proxima_data && form.proxima_data < form.data_registo) {
        throw new Error('A próxima data não pode ser anterior à data do registo.');
      }
              ...
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input required type="date" min={form.data_registo} value={form.proxima_data} onChange={e => setForm({...form, proxima_data: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none text-slate-600 focus:border-[#0d9488]" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
