            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-7 gap-px bg-slate-100 border border-slate-100 rounded-3xl overflow-hidden shadow-inner">
                  {blanks.map(b => <div key={`b-${b}`} className="bg-slate-50/40 min-h-[40px] md:min-h-[120px]"></div>)}
                  {days.map(day => {
                    ...
                    return (
                      <div key={day} className="bg-white min-h-[40px] md:min-h-[120px] p-1 md:p-2 relative group hover:bg-slate-50 transition-colors">
                        <span className={`inline-flex items-center justify-center w-6 h-6 md:w-8 md:h-8 text-[10px] md:text-xs font-black rounded-xl mb-1 md:mb-2 ${isHoje ? 'bg-[#0d9488] text-white shadow-md shadow-teal-100' : 'text-slate-500'}`}>
                          {day}
                        </span>
                        
                        <div className="hidden md:flex flex-col gap-1.5 overflow-y-auto overflow-x-hidden max-h-[80px] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                          {eventosDia.map(ev => (
                            <div 
                              key={ev.id} 
                          ...
                          <div className="flex md:hidden flex-wrap gap-0.5 mt-0.5">
                            {eventosDia.slice(0, 3).map(ev => (
                              <div
                                key={ev.id}
                                onClick={() => setSelectedEventDetail(ev)}
                                className={`w-1.5 h-1.5 rounded-full cursor-pointer ${getTipoEstilo(ev.tipo, ev.estado).includes('teal') ? 'bg-teal-500' : getTipoEstilo(ev.tipo, ev.estado).includes('rose') ? 'bg-rose-500' : getTipoEstilo(ev.tipo, ev.estado).includes('amber') ? 'bg-amber-500' : 'bg-slate-400'}`}
                              />
                            ))}
                          </div>
                        )}
