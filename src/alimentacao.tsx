  ...
  const handleAddStock = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('alimentacao').insert([{
        ...
        stock_atual: parseFloat(form.stock_atual),
        stock_total: parseFloat(form.stock_total || form.stock_atual),
        porcao_diaria: parseFloat(form.porcao_diaria) || 0,
  ...
  const handleUpdateStockValue = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('alimentacao').update({ 
        id_animal: form.id_animal,
        nome_racao: form.nome_racao,
        stock_atual: parseFloat(parseFloat(form.stock_atual).toFixed(2)),
        stock_total: parseFloat(parseFloat(form.stock_total).toFixed(2)),
        porcao_diaria: parseFloat(form.porcao_diaria),
        link_compra: form.link_compra || null,
        foto_url: publicUrl
            ...
            <div className="bg-white rounded-[2rem] p-8 w-full max-w-md shadow-xl relative max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleAddStock} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input required type="number" step="0.1" placeholder="Stock (kg)" value={form.stock_atual} onChange={e => setForm({...form, stock_atual: e.target.value, stock_total: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none" />
                  <input required type="number" placeholder="Dose (g)" value={form.porcao_diaria} onChange={e => setForm({...form, porcao_diaria: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none" />
                </div>
              ...
              <form onSubmit={handleUpdateStockValue} className="flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                     ...
                     <input required type="number" step="0.1" value={form.stock_atual} onChange={e => setForm({...form, stock_atual: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none focus:border-[#0d9488]" />
                  </div>
                  <div className="flex flex-col gap-1">
                     <span className="text-[10px] font-black text-slate-400 uppercase ml-2">Stock Total (kg)</span>
                     <input required type="number" step="0.1" value={form.stock_total} onChange={e => setForm({...form, stock_total: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none focus:border-[#0d9488]" />
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-black text-slate-400 uppercase ml-2">Dose Diária (g)</span>
                    <input required type="number" placeholder="Dose (g)" value={form.porcao_diaria} onChange={e => setForm({...form, porcao_diaria: e.target.value})} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold outline-none focus:border-[#0d9488]" />
                </div>
