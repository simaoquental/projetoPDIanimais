...
export default function Animais() {
  ...
  const handleSavePet = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const pesoFinal = parseFloat(novoPesoValor);
      const idadeFinal = novaIdade !== '' ? parseInt(novaIdade) : null;
      if (isNaN(pesoFinal) || pesoFinal <= 0) throw new Error('O peso deve ser um valor positivo.');
      if (idadeFinal !== null && (isNaN(idadeFinal) || idadeFinal < 0)) throw new Error('A idade deve ser um valor válido.');
  ...
  const handleQuickWeightSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNaN(pesoNumerico) || pesoNumerico <= 0) throw new Error('O peso deve ser um valor positivo.');
  ...
  const resetForm = () => {
    ...
    setNovaIdade(''); setNovoTipoSangue(''); 
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSavePet} className="space-y-4">
                ...
                <select required value={novaRaca} onChange={e => setNovaRaca(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 outline-none focus:ring-2 focus:ring-teal-500">
                  <option value="">Selecionar Raça</option>
                  {(novaEspecie === 'Cão' ? racasCaes : racasGatos).map((r) => (
                    <option key={r} value={r}>{r}</option>
                  ))}
                </select>

                <div className="grid grid-cols-2 gap-4">
                  <input required type="number" min="0" placeholder="Idade (anos)" value={novaIdade} onChange={e => setNovaIdade(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 outline-none focus:ring-2 focus:ring-teal-500" />
                  <input required type="number" min="0.1" step="0.1" placeholder="Peso (kg)" value={novoPesoValor} onChange={e => setNovoPesoValor(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm relative shadow-2xl">
              ...
              <form onSubmit={handleQuickWeightSave} className="space-y-4">
                <input required type="number" min="0.1" step="0.1" placeholder="Novo Peso em kg" value={novoPesoRapido} onChange={e => setNovoPesoRapido(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-center text-lg font-bold outline-none focus:ring-2 focus:ring-teal-500" />
